...
def avaliar_no_teste(model, test_loader, criterion, device, caminho_melhor_modelo, classes):
    ...
    test_loss, test_macro_f1, test_labels, test_preds = evaluate(model, test_loader, criterion, device)

    print(f"Loss no teste: {test_loss:.4f}")
    print(f"Macro F1 no teste: {test_macro_f1:.4f}\n")

    print("Relatorio por classe (teste):")
    print(classification_report(test_labels, test_preds, target_names=classes, zero_division=0))

    return test_macro_f1, test_labels, test_preds
